<?php
  ...
  if(!empty($_GET['id']))
  {
      ...
      $sqlSelect = "SELECT * FROM tbl_clientes WHERE id=$id";

      $result = $conexao->query($sqlSelect);

      if($result->num_rows > 0)
      {
          $sqlDelete = "DELETE FROM tbl_clientes WHERE id=$id";
  ...
  if(!empty($_GET['search'])){
    $data = $_GET['search'];
    $sql = "SELECT * FROM tbl_clientes WHERE id LIKE '%$data%' or nome LIKE '%$data%' or cpf LIKE '%$data%'";
    $result = $conexao->query($sql);
    ...
    <?php
        while($user_data = mysqli_fetch_assoc($result))
        {
          ...
          $data_brasileira = date("d/m/Y",$data_timestamp);
            ...
            <a class='btn btn-sm btn-danger' href='clientes.php?id=$user_data[id]' title='Deletar'>
